...
class I3D(torch.nn.Module):
    ...
    def forward(self, inp, stop=None):
        # Preprocessing
        out = self.conv3d_1a_7x7(inp)
        out = self.maxPool3d_2a_3x3(out)
        #print('2a: %s' % str(out.size()))
        if stop == '2a':
            return out      # [64, 16, 56, 56]
        out = self.conv3d_2b_1x1(out)
        out = self.conv3d_2c_3x3(out)
        out = self.maxPool3d_3a_3x3(out)
        #print('3a: %s' % str(out.size()))
        if stop == '3a':
            return out      # [192, 16, 28, 28]
        out = self.mixed_3b(out)
        out = self.mixed_3c(out)
        out = self.maxPool3d_4a_3x3(out)
        #print('4a: %s' % str(out.size()))
        if stop == '4a':
            return out      # [480, 8, 14, 14]
        ...
        #print('5a: %s' % str(out.size()))
        if stop == '5a':
            return out      # [832, 4, 7, 7]    -> 11 * 8
        out = self.mixed_5b(out)
        out = self.mixed_5c(out)
        #print('5c: %s' % str(out.size()))
        if stop == '5c':
            return out      # [1024, 4, 7, 7]
        out = self.avg_pool(out)
        #print('avg: %s' % str(out.size()))
        if stop == 'avg':
            return out      # [1024, 3, 1, 1]   -> 5 2
